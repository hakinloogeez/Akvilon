...
int nAnonymizeAkvilonCoinAmount = 1000;
...
#else
    const char* pszModule = "AkvilonCoin";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\AkvilonCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\AkvilonCoin
    // Mac: ~/Library/Application Support/AkvilonCoin
    // Unix: ~/.AkvilonCoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "AkvilonCoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "AkvilonCoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "AkvilonCoind.pid"));
