...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("AkvilonCoin"))
        return false;
...
{
    // Convert AkvilonCoin:// to AkvilonCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("AkvilonCoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 11, "AkvilonCoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "AkvilonCoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for AkvilonCoin.lnk
...
{
    return GetAutostartDir() / "AkvilonCoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        // Write a AkvilonCoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=AkvilonCoin";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the AkvilonCoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add AkvilonCoin app to startup item list
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("AkvilonCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  AkvilonCoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("AkvilonCoin-Qt"));
