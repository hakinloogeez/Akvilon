...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeAkvilonCoinAmount"))
        settings.setValue("nAnonymizeAkvilonCoinAmount", 1000);
    nAnonymizeAkvilonCoinAmount = settings.value("nAnonymizeAkvilonCoinAmount").toLongLong();
    if (settings.contains("nDarksendRounds"))
        SoftSetArg("-darksendrounds", settings.value("nDarksendRounds").toString().toStdString());
    if (settings.contains("nAnonymizeAkvilonCoinAmount"))
        SoftSetArg("-anonymizetraveamount", settings.value("nAnonymizeAkvilonCoinAmount").toString().toStdString());
...
QVariant OptionsModel::data(const QModelIndex & index, int role) const
{
    if(role == Qt::EditRole)
    {
        ...
        case AnonymizeAkvilonCoinAmount:
            return QVariant(nAnonymizeAkvilonCoinAmount);
        case UseBlackTheme:
...
bool OptionsModel::setData(const QModelIndex & index, const QVariant & value, int role)
{
    ...
    if(role == Qt::EditRole)
    {
        ...
        case DarksendRounds:
            ...
            break;
        case AnonymizeAkvilonCoinAmount:
            nAnonymizeAkvilonCoinAmount = value.toInt();
            settings.setValue("nAnonymizeAkvilonCoinAmount", nAnonymizeAkvilonCoinAmount);
            emit AnonymizeAkvilonCoinAmountChanged(nAnonymizeAkvilonCoinAmount);
            break;
        default:
            break;
        }
