...
void OverviewPage::updateDarksendProgress()
{
    ...
    QString strAnonymizeAkvilonCoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeAkvilonCoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if(currentBalance == 0)
    {
        ...
        strAnonymizeAkvilonCoinAmount = strAnonymizeAkvilonCoinAmount.remove(strAnonymizeAkvilonCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeAkvilonCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    ...
    if(nMaxToAnonymize > nAnonymizeAkvilonCoinAmount*COIN) nMaxToAnonymize = nAnonymizeAkvilonCoinAmount*COIN;

    if(nMaxToAnonymize == 0) return;

    if(nMaxToAnonymize >= nAnonymizeAkvilonCoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                          .arg(strAnonymizeAkvilonCoinAmount));
        strAnonymizeAkvilonCoinAmount = strAnonymizeAkvilonCoinAmount.remove(strAnonymizeAkvilonCoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeAkvilonCoinAmount + " / " + tr("%n Rounds", "", nDarksendRounds);
    } else {
...
void OverviewPage::toggleDarksend(){
    ...
    if(!fEnableDarksend){
        ...
        if(nAnonymizeAkvilonCoinAmount == 0){
